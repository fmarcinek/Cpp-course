...
template< int M1, int L1, int T1, int I1,
          int M2, int L2, int T2, int I2 >
si<M1+M2, L1+L2, T1+T2, I1+I2 > operator * ( si<M1,L1,T1,I1> q1, si<M2,L2,T2,I2> q2 ) {
    si<M1+M2, L1+L2, T1+T2, I1+I2 > res (q1.val * q2.val);
    return res;
}
...
si<M,L,T,I> operator - ( si<M,L,T,I> q ) {
    si<M,L,T,I> res (-q.val);
    return res;
}

template< int M, int L, int T, int I >
si<M,L,T,I> operator + ( si<M,L,T,I> q1, si<M,L,T,I> q2 ) {
    si<M,L,T,I> res (q1.val + q2.val);
    return res;
}
